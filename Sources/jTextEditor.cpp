...
namespace Kiwi
{
	...
	//==============================================================================
	class jTextEditor::InsertAction  : public UndoableAction
	{
	public:
	...
	//==============================================================================
	class jTextEditor::RemoveAction  : public UndoableAction
	{
	public:
		RemoveAction(jTextEditor& ed,
	...
	//==============================================================================
	class jTextEditor::TextHolderComponent  : public Component,
	public Timer,
	public ValueListener
	{
	public:
		TextHolderComponent(jTextEditor& ed)  : owner(ed)
		{
	...
	//==============================================================================
	class jTextEditorViewport  : public Viewport
	{
	public:
		jTextEditorViewport(jTextEditor& ed)
	...
	private:
		...
		JUCE_DECLARE_NON_COPYABLE(jTextEditorViewport)
	};
	
	//==============================================================================
	namespace jTextEditorDefs
	{
	...
	dragType(notDragging)
	{
		...
		addAndMakeVisible(viewport = new jTextEditorViewport(*this));
	...
	//==============================================================================
	void jTextEditor::newTransaction()
	{
		lastTransactionTime = Time::getApproximateMillisecondCounter();
		undoManager.beginNewTransaction();
	}
	
	bool jTextEditor::undoOrRedo(const bool shouldUndo)
	{
		if(! isReadOnly())
		{
			newTransaction();
	...
	bool jTextEditor::undo()     { return undoOrRedo(true); }
	bool jTextEditor::redo()     { return undoOrRedo(false); }
	
	//==============================================================================
	void jTextEditor::setMultiLine(const bool shouldBeMultiLine,
	...
	bool jTextEditor::isMultiLine() const
	{
		return multiline;
	}
	
	void jTextEditor::setScrollbarsShown(bool shown)
	{
	...
	void jTextEditor::setReadOnly(const bool shouldBeReadOnly)
	{
	...
	bool jTextEditor::isTextInputActive() const
	{
		return ! isReadOnly();
	}
	
	void jTextEditor::setReturnKeyStartsNewLine(const bool shouldStartNewLine)
	{
		returnKeyStartsNewLine = shouldStartNewLine;
	}
	
	void jTextEditor::setTabKeyUsedAsCharacter(const bool shouldTabKeyBeUsed)
	{
		tabKeyUsed = shouldTabKeyBeUsed;
	}
	
	void jTextEditor::setPopupMenuEnabled(const bool b)
	{
		popupMenuEnabled = b;
	}
	
	void jTextEditor::setSelectAllWhenFocused(const bool b)
	{
	...
	void jTextEditor::colourChanged()
	{
		setOpaque(findColour(backgroundColourId).isOpaque());
		repaint();
	}
	
	void jTextEditor::lookAndFeelChanged()
	{
		if(isCaretVisible())
		{
	...
	void jTextEditor::setCaretVisible(const bool shouldCaretBeVisible)
	{
	...
	void jTextEditor::updateCaretPosition()
	{
		if(caret != nullptr)
			caret->setCaretPosition(getCaretRectangle().translated(leftIndent, topIndent));
	}
	
	jTextEditor::LengthAndCharacterRestriction::LengthAndCharacterRestriction(int maxLen, const String& chars)
	: allowedCharacters(chars), maxLength(maxLen)
	{}
	
	String jTextEditor::LengthAndCharacterRestriction::filterNewText(sjTextEditor editor, const String& newInput)
	{
		String t(newInput);
		...
		{
			if(maxLength > 0)
				t = t.substring(0, maxLength -(editor->getTotalNumChars() - editor->getHighlightedRegion().getLength()));
		}
		
		return t;
	}
	
	void jTextEditor::setInputFilter(InputFilter* newFilter, bool takeOwnership)
	{
		inputFilter.set(newFilter, takeOwnership);
	}
	
	void jTextEditor::setInputRestrictions(const int maxLen,
										   const String& chars)
	{
		setInputFilter(new LengthAndCharacterRestriction(maxLen, chars), true);
	}
	
	void jTextEditor::setTextToShowWhenEmpty(const String& text, Colour colourToUse)
	{
		textToShowWhenEmpty = text;
		colourForTextWhenEmpty = colourToUse;
	}
	
	void jTextEditor::setScrollBarThickness(const int newThicknessPixels)
	{
		viewport->setScrollBarThickness(newThicknessPixels);
	}
	
	//==============================================================================
	void jTextEditor::clear()
	{
	...
	void jTextEditor::setText(const String& newText,
	...
	//==============================================================================
	void jTextEditor::updateValueFromText()
	{
	...
	Value& jTextEditor::getTextValue()
	{
		updateValueFromText();
		return textValue;
	}
	
	void jTextEditor::textWasChangedByValue()
	{
	...
	//==============================================================================
	void jTextEditor::textChanged()
	{
		updateTextHolderSize();
		
		if(listeners.size() > 0)
			postCommandMessage(jTextEditorDefs::textChangeMessageId);
	...
	void jTextEditor::returnPressed()    { postCommandMessage(jTextEditorDefs::returnKeyMessageId); }
	void jTextEditor::escapePressed()    { postCommandMessage(jTextEditorDefs::escapeKeyMessageId); }
	
	void jTextEditor::addListener(jTextEditor::Listener* const l)      { listeners.add(l); }
	void jTextEditor::removeListener(jTextEditor::Listener* const l)   { listeners.remove(l); }
	
	//==============================================================================
	void jTextEditor::timerCallbackInt()
	{
	...
	void jTextEditor::repaintText(const Range<int> range)
	{
		if(! range.isEmpty())
		{
	...
	//==============================================================================
	void jTextEditor::moveCaret(int newCaretPos)
	{
		if(newCaretPos < 0)
			newCaretPos = 0;
		else
	...
	int jTextEditor::getCaretPosition() const
	{
		return caretPosition;
	}
	
	void jTextEditor::setCaretPosition(const int newIndex)
	{
		moveCaretTo(newIndex, false);
	}
	
	void jTextEditor::moveCaretToEnd()
	{
		moveCaretTo(std::numeric_limits<int>::max(), false);
	}
	
	void jTextEditor::scrollEditorToPositionCaret(const int desiredCaretX,
	...
	int jTextEditor::getTextWidth() const    { return textHolder->getWidth(); }
	int jTextEditor::getTextHeight() const   { return textHolder->getHeight(); }
	
	void jTextEditor::setIndents(const int newLeftIndent, const int newTopIndent)
	{
		leftIndent = newLeftIndent;
		topIndent  = newTopIndent;
	}
	
	void jTextEditor::setBorder(const BorderSize<int>& border)
	{
		borderSize = border;
		resized();
	}
	
	BorderSize<int> jTextEditor::getBorder() const
	{
		return borderSize;
	}
	
	void jTextEditor::setScrollToShowCursor(const bool shouldScrollToShowCursor)
	{
		keepCaretOnScreen = shouldScrollToShowCursor;
	}
	
	void jTextEditor::scrollToMakeSureCursorIsVisible()
	{
		updateCaretPosition();
	...
	void jTextEditor::moveCaretTo(const int newPosition, const bool isSelecting)
	{
		if(isSelecting)
		{
	...
	int jTextEditor::getTextIndexAt(const int x, const int y)
	{
	...
	void jTextEditor::insertTextAtCaret(const String& t)
	{
		String newText = t;
		...
		{
			newText = inputFilter->filterNewText(shared_from_this(), newText);
		}
	...
	void jTextEditor::setHighlightedRegion(const Range<int>& newSelection)
	{
		moveCaretTo(newSelection.getStart(), false);
		moveCaretTo(newSelection.getEnd(), true);
	}
	
	Range<int> jTextEditor::getHighlightedRegion() const
	{
		return selection;
	}
	
	//==============================================================================
	void jTextEditor::copy()
	{
	...
	//==============================================================================
	void jTextEditor::drawContent(Graphics& g)
	{
		const float wordWrapWidth = getWordWrapWidth();
		...
		if(wordWrapWidth > 0)
		{
			...
			for(int j = underlinedSections.size(); --j >= 0;)
			{
				...
				Iterator i2(sections, wordWrapWidth);
	...
	void jTextEditor::paint(Graphics& g)
	{
		g.fillAll(findColour(jTextEditor::backgroundColourId));
	}
	
	void jTextEditor::paintOverChildren(Graphics& g)
	{
			...
			&& getTotalNumChars() == 0)
		{
			...
			if(isMultiLine())
			{
				g.drawText(textToShowWhenEmpty, getLocalBounds(), Justification::centred, true);
			}
			else
			{
				g.drawText(textToShowWhenEmpty, leftIndent, 0, viewport->getWidth() - leftIndent, getHeight(), Justification::centredLeft, true);
			}
		}
		...
		if(isEnabled())
		{
			...
			{
				g.setColour(findColour(jTextEditor::focusedOutlineColourId));
				g.drawRect(0, 0, getWidth(), getHeight(), 2);
			}
			else
			{
				g.setColour(findColour(jTextEditor::outlineColourId));
	...
	//==============================================================================
	void jTextEditor::addPopupMenuItems(PopupMenu& m, const MouseEvent*)
	{
	...
	void jTextEditor::performPopupMenuAction(const int menuItemID)
	{
		switch(menuItemID)
		{
	...
	static void textEditorMenuCallback(int menuResult, jTextEditor* editor)
	{
	...
	//==============================================================================
	void jTextEditor::mouseDown(const MouseEvent& e)
	{
	...
	void jTextEditor::mouseDrag(const MouseEvent& e)
	{
		if(wasFocused || ! selectAllTextWhenFocused)
			if(!(popupMenuEnabled && e.mods.isPopupMenu()))
	...
	void jTextEditor::mouseUp(const MouseEvent& e)
	{
		newTransaction();
		textHolder->restartTimer();
	...
	void jTextEditor::mouseDoubleClick(const MouseEvent& e)
	{
	...
	void jTextEditor::mouseWheelMove(const MouseEvent& e, const MouseWheelDetails& wheel)
	{
	...
	//==============================================================================
	bool jTextEditor::moveCaretWithTransaction(const int newPos, const bool selecting)
	{
		newTransaction();
	...
	bool jTextEditor::moveCaretLeft(bool moveInWholeWordSteps, bool selecting)
	{
	...
	bool jTextEditor::moveCaretRight(bool moveInWholeWordSteps, bool selecting)
	{
		int pos = getCaretPosition();
	...
	bool jTextEditor::moveCaretUp(bool selecting)
	{
		if(! isMultiLine())
	...
	bool jTextEditor::moveCaretDown(bool selecting)
	{
		if(! isMultiLine())
	...
	bool jTextEditor::pageUp(bool selecting)
	{
		if(! isMultiLine())
			return moveCaretToStartOfLine(selecting);
	...
	bool jTextEditor::pageDown(bool selecting)
	{
		if(! isMultiLine())
			return moveCaretToEndOfLine(selecting);
	...
	void jTextEditor::scrollByLines(int deltaLines)
	{
		if(ScrollBar* scrollbar = viewport->getVerticalScrollBar())
			scrollbar->moveScrollbarInSteps(deltaLines);
	}
	
	bool jTextEditor::scrollDown()
	{
		scrollByLines(-1);
		return true;
	}
	
	bool jTextEditor::scrollUp()
	{
		scrollByLines(1);
		return true;
	}
	
	bool jTextEditor::moveCaretToTop(bool selecting)
	{
	...
	bool jTextEditor::moveCaretToEnd(bool selecting)
	{
	...
	bool jTextEditor::deleteBackwards(bool moveInWholeWordSteps)
	{
		if(moveInWholeWordSteps)
	...
	bool jTextEditor::deleteForwards(bool /*moveInWholeWordSteps*/)
	{
	...
	bool jTextEditor::copyToClipboard()
	{
		newTransaction();
		copy();
		return true;
	...
	bool jTextEditor::cutToClipboard()
	{
		newTransaction();
		copy();
		cut();
	...
	bool jTextEditor::pasteFromClipboard()
	{
		newTransaction();
		paste();
		return true;
	...
	//==============================================================================
	void jTextEditor::setEscapeAndReturnKeysConsumed(bool shouldBeConsumed) noexcept
	{
		consumeEscAndReturnKeys = shouldBeConsumed;
	}
	
	bool jTextEditor::keyPressed(const KeyPress& key)
	{
		...
		if (writer)
		{
			...
			{
				if(! TextEditorKeyMapper<jTextEditor>::invokeKeyFunction(*this, key))
				{
	...
	bool jTextEditor::keyStateChanged(const bool isKeyDown)
	{
		if(! isKeyDown)
			return false;
	...
	//==============================================================================
	void jTextEditor::focusGained(FocusChangeType)
	{
		newTransaction();
	...
	void jTextEditor::focusLost(FocusChangeType)
	{
		newTransaction();
		...
		postCommandMessage(jTextEditorDefs::focusLossMessageId);
		repaint();
	}
	
	//==============================================================================
	void jTextEditor::resized()
	{
	...
	void jTextEditor::handleCommandMessage(const int commandId)
	{
		Component::BailOutChecker checker(this);
		
		switch(commandId)
		{
			case jTextEditorDefs::textChangeMessageId:
				listeners.callChecked(checker, &jTextEditor::Listener::textEditorTextChanged, shared_from_this());
				break;
				
			case jTextEditorDefs::returnKeyMessageId:
				listeners.callChecked(checker, &jTextEditor::Listener::textEditorReturnKeyPressed, shared_from_this());
				break;
				
			case jTextEditorDefs::escapeKeyMessageId:
				listeners.callChecked(checker, &jTextEditor::Listener::textEditorEscapeKeyPressed, shared_from_this());
				break;
				
			case jTextEditorDefs::focusLossMessageId:
				updateValueFromText();
				listeners.callChecked(checker, &jTextEditor::Listener::textEditorFocusLost, shared_from_this());
				break;
	...
	void jTextEditor::enablementChanged()
	{
		repaint();
	}
	
	void jTextEditor::setTemporaryUnderlining(const Array<Range<int> >& newUnderlinedSections)
	{
		underlinedSections = newUnderlinedSections;
		repaint();
	}
	
	auto jTextEditor::getKeyboardType() -> VirtualKeyboardType
	{
		return keyboardType;
	}
	
	//==============================================================================
	UndoManager* jTextEditor::getUndoManager() noexcept
	{
		return readOnly ? nullptr : &undoManager;
	}
	
	void jTextEditor::clearInternal(UndoManager* const um)
	{
							 ...
							 const int caretPositionToMoveTo)
	{
		if(text.isNotEmpty())
		{
			if(um != nullptr)
			{
				if(um->getNumActionsInCurrentTransaction() > jTextEditorDefs::maxActionsPerTransaction)
					newTransaction();
	...
	void jTextEditor::reinsert(const int insertIndex, const OwnedArray<UniformTextSection>& sectionsToInsert)
	{
		int index = 0;
		int nextIndex = 0;
	...
	void jTextEditor::remove(Range<int> range, UndoManager* const um, const int caretPositionToMoveTo)
	{
		if(! range.isEmpty())
		{
			int index = 0;
	...
	//==============================================================================
	String jTextEditor::getText() const
	{
	...
	String jTextEditor::getTextInRange(const Range<int>& range) const
	{
		if(range.isEmpty())
			return String::empty;
	...
	String jTextEditor::getHighlightedText() const
	{
		return getTextInRange(selection);
	}
	
	int jTextEditor::getTotalNumChars() const
	{
		if(totalNumChars < 0)
		{
			totalNumChars = 0;
	...
	bool jTextEditor::isEmpty() const
	{
		return getTotalNumChars() == 0;
	}
	
	void jTextEditor::getCharPosition(const int index, float& cx, float& cy, float& lineHeight) const
	{
		const float wordWrapWidth = getWordWrapWidth();
	...
	int jTextEditor::indexAtPosition(const float x, const float y)
	{
		const float wordWrapWidth = getWordWrapWidth();
	...
	//==============================================================================
	int jTextEditor::findWordBreakAfter(const int position) const
	{
		...
		const int type = jTextEditorDefs::getCharacterCategory(t[i]);
		
		while(i < totalLength && type == jTextEditorDefs::getCharacterCategory(t[i]))
			++i;
	...
	int jTextEditor::findWordBreakBefore(const int position) const
	{
		if(position <= 0)
			return 0;
		...
		if(i > 0)
		{
			const int type = jTextEditorDefs::getCharacterCategory(t [i - 1]);
			
			while(i > 0 && type == jTextEditorDefs::getCharacterCategory(t [i - 1]))
				--i;
		}
	...
	//==============================================================================
	void jTextEditor::splitSection(const int sectionIndex, const int charToSplitAt)
	{
	...
	void jTextEditor::coalesceSimilarSections()
	{
