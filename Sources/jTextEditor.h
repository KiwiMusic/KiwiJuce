...
#ifndef __DEF_KIWI_JTEXTEDITOR__
#define __DEF_KIWI_JTEXTEDITOR__
...
namespace Kiwi
{
	...
	/**
	 ...
	 A jTextEditor can either be in single- or multi-line mode, and supports mixed
	 fonts and colours.
	 
	 @see jTextEditor::Listener, Label
	 */
	class jTextEditor : public Component, public TextInputTarget, public enable_shared_from_this<jTextEditor>
	{
	private:
	...
	public:
		...
		/** Changes the behaviour of the return key.
		 ...
		 it will trigger a call to the jTextEditor::Listener::textEditorReturnKeyPressed()
		...
		//==============================================================================
		/**
		 Receives callbacks from a jTextEditor component when it changes.
		 
		 @see jTextEditor::addListener
		 */
		class Listener
		{
		public:
			...
			virtual void textEditorTextChanged(sjTextEditor editor) {}
			
			/** Called when the user presses the return key. */
			virtual void textEditorReturnKeyPressed(sjTextEditor editor) {}
			
			/** Called when the user presses the escape key. */
			virtual void textEditorEscapeKeyPressed(sjTextEditor editor) {}
			
			/** Called when the text editor loses focus. */
			virtual void textEditorFocusLost(sjTextEditor editor) {}
		};
		...
		/** Sets the entire content of the editor.
		 ...
		 @code setColour(jTextEditor::textColourId, ...);
		 @endcode
		...
		/** Returns the total width of the text, as it is currently laid-out.
		 ...
		 This may be larger than the size of the jTextEditor, and can change when
		 the jTextEditor is resized or the text changes.
		 */
		...
		//==============================================================================
		/** Base class for input filters that can be applied to a jTextEditor to restrict
		...
		public:
			 ...
			 */
			virtual String filterNewText(sjTextEditor editor, const String& newInput) = 0;
		};
		
		/** An input filter for a jTextEditor that limits the length of text and/or the
		...
		private:
			...
			String filterNewText(sjTextEditor editor, const String& newInput) override;
		...
		/** This abstract base class is implemented by LookAndFeel classes to provide
		 jTextEditor drawing functionality.
		 */
		struct JUCE_API  LookAndFeelMethods
		{
			virtual ~LookAndFeelMethods() {}
			
			virtual void filljTextEditorBackground(Graphics&, int width, int height, jTextEditor&) = 0;
			virtual void drawjTextEditorOutline(Graphics&, int width, int height, jTextEditor&) = 0;
	...
	private:
		...
		friend class jTextEditorViewport;
		...
		JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(jTextEditor)
	};
	
	typedef shared_ptr<jTextEditor>			sjTextEditor;
	typedef weak_ptr<jTextEditor>			wjTextEditor;
	typedef shared_ptr<const jTextEditor> 	scjTextEditor;
	typedef weak_ptr<const jTextEditor> 	wcjTextEditor;
}
#endif
